...
<script type="text/javascript">
	$(function(){
		$("#searchBtn").click(function(){
			self.location = "sendList"+"${pagingMaker.makeUri(1)}"
			...
			<div id="txt">
				...
				<div class="layout">
                    ...
                    <div class="notice_list">
                        <ul>
                            <li class="thead">
                                <strong class="no" style="width: 10%;">번호</strong>
                                <strong class="subject" style="width: 30%;">제목</strong>
                                <strong class="writer" style="width: 30%;">받는사람</strong>
                                <strong class="date" style="width: 30%;">보낸일자</strong>
                            ...
                            <c:forEach var="sdto" items="${slist }">
	                            <li>
	                                ...
	                                <p class="subject align_left">
	                                    <a href="/message/sendreadPage${pagingMaker.makeSearch(pagingMaker.cri.page) }&message_no=${sdto.message_no}" class="new">
	                                        <strong>
	                                            ${sdto.message_title }
	                                ...
	                                <p class="writer">${sdto.message_id }</p>  
	                                <p class="date"><fmt:formatDate value="${sdto.message_date }" pattern="yyyy/MM/dd MM:mm:ss" /></p>
<%-- 	                                <p class="counter">${mdto.board_readcnt }</p> --%>
	                                <p class="file_atch"></p>
	                            </li>
                    ...
                    <div class="page">
                        <c:if test="${pagingMaker.prev }">
						  <a href="sendList${pagingMaker.makeSearch(pagingMaker.startPage - 1 ) }" class="page_left">
						  이전</a>
						</c:if>
                        <c:forEach begin="${pagingMaker.startPage }" end="${pagingMaker.endPage }" var="pNum" >
						  <a href="sendList${pagingMaker.makeSearch(pNum) }" <c:if test="${sc.page eq pNum}">class="on"</c:if> >${pNum }</a>
						</c:forEach>
                        <!-- <a href="#" class="on" title="현재 페이지">5</a> --> 
                        <c:if test="${pagingMaker.next && pagingMaker.endPage > 0}">
						  <a href="sendList${pagingMaker.makeSearch(pagingMaker.endPage + 1) }" class="page_right">
						  	다음
						  </a>
						</c:if>
                    </div>
