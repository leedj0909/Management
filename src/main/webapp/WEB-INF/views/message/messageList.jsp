...
<script type="text/javascript">
	$(function(){
		$("#searchBtn").click(function(){
			self.location = "messageList"+"${pagingMaker.makeUri(1)}"
                    ...
                    <div class="notice_list">
                        <ul>
                            <li class="thead">
                                ...
                                <strong class="writer" style="width: 140px;">보낸사람</strong>
                                <strong class="date">보낸일자</strong>
...
<%--                             <c:forEach var="bdto" items="${list }" varStatus="index"> --%>
	                            <li>
	                                ...
	                                <p class="subject align_left">
	                                    <a href="/message/readPage${pagingMaker.makeSearch(pagingMaker.cri.page) }&message_no=${mdto.message_no}" class="new">
	                                        <strong>
	                                            ${mdto.message_title }
	                                ...
	                                <p class="writer">${mdto.message_id }</p>  
	                                <p class="date"><fmt:formatDate value="${mdto.message_date }" pattern="yyyy/MM/dd" /></p>
<%-- 	                                <p class="counter">${mdto.board_readcnt }</p> --%>
                    ...
                    <c:if test="${login != null }">
                        <a href="/message/sendList" class="btn">보낸메세지함</a>
                        <a href="/message/write" class="btn">쪽지 보내기</a>
                    </c:if>
                    </div>
                    <div class="page">
                        <c:if test="${pagingMaker.prev }">
						  <a href="messageList${pagingMaker.makeSearch(pagingMaker.startPage - 1 ) }" class="page_left">
						  이전</a>
						</c:if>
                        <c:forEach begin="${pagingMaker.startPage }" end="${pagingMaker.endPage }" var="pNum" >
						  <a href="messageList${pagingMaker.makeSearch(pNum) }" <c:if test="${sc.page eq pNum}">class="on"</c:if> >${pNum }</a>
						</c:forEach>
                        <!-- <a href="#" class="on" title="현재 페이지">5</a> --> 
                        <c:if test="${pagingMaker.next && pagingMaker.endPage > 0}">
						  <a href="messageList${pagingMaker.makeSearch(pagingMaker.endPage + 1) }" class="page_right">
						  	다음
						  </a>
						</c:if>
                    </div>
